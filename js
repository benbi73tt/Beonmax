...
//ANCHOR ``-в ОБРАТНЫЕ КОВЫЧКИ можно добавлять переменные с помощью ${name},где name=Иван.
// alert(`Результат: ${1+2}`);
//TODO let check=4>1;//(boolean)Булевая функция, которая вернет только ДА или НЕТ(trur,false).
//TODO alert(check);//ПРОВЕРКА
//null- означает пусто, ничего или значение неизвестно
//TODO let x;//при вызове Х будет выдано UNDEFINED
...
//STUB let Old=confirm("Ты старше 18 лет?");
// alert(Old);
//REVIEW 
...
// REVIEW let login=prompt("login"," ");
// let message=(login=='Сотрудник')?'Привет'
// :(login=='Директор')?'Здравствуйте'
// :(login==' ')?'Нет логина':'';
// alert(message);

//REVIEW let who=prompt("кто там"," ");
// if(who=="Admin"){
...
// }
//TODO для выхода из ДВУХ циклов for сразу
...
//     alert(i);
// }
// REVIEW let a=prompt("Cколько будет 2+2"," " );
...
//REVIEW
// let ask=(ques,yes,no)=>{if (confirm(ques))yes();
//      else no();}
// ask('ВЫ согласны?',()=>{alert("YES");},()=>{alert("NO");});

//TODO debugger; ПРИОСТАНАВЛИВАЕТ ОТКЛАДКУ КОДА
...
// Object.assign(user,per1,per2,{name:'Toni'});//Свойство объекта переапишется
// alert(user.name);
