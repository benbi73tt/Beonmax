...
function calc() {
  ...
  persons.addEventListener('change', function () {
    ...
    if (persons.value == '' && persons.value == 0 && daysSum == 0) {
      //если второй инпут не заполнен,результат должен быть равен 0
      totalValue.innerHTML = 0;
    } else {
  ...
  restDays.addEventListener('change', function () {
    ...
    if (persons.value == '' && persons.value == 0 && daysSum == 0) {
      //если первый инпут не заполнен,результат должен быть равен 0
      totalValue.innerHTML = 0;
    } else {
      totalValue.innerHTML = total;
    }
...
function form() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); //???????????????????СЕРВЕР
...
function modal() {
  var more = document.querySelector('.more'),
      ...
      close = document.querySelector('.popup-close'); //style css

  more.addEventListener('click', function () {
    overlay.style.display = 'block';
    this.classList.add('more-splash'); //запрещает прокручить страницу при открытом модальном окне
...
function slider() {
  ...
  function showSlides(n) {
    //n-переключатель слайдов
    if (n > slides.length) {
      //при клике на последнем слайде 'далее' возвращает на первый слайд
      slideIndex = 1;
    }
  ...
  dotsWrap.addEventListener('click', function (event) {
    for (var i = 0; i < dots.length + 1; i++) {
      if (event.target.classList.contains('dot') && event.target == dots[i - 1]) {
        //проверить есть ли у элемента класс dot
        currentSlides(i);
      }
    }
  });
...
function tabs() {
  ...
  info.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains('info-header-tab')) {
      //Если кликнутый нами Taб вообще принадлежит к нашему классу
      for (var i = 0; i < tab.length; i++) {
        if (target == tab[i]) {
          //Ищем Номер Таба, который мы кликнули
...
function timer() {
  ...
  function setClock(id, endtime) {
    //сздает элементы, берет со страницы
    ...
    function upDateClock() {
      //функция вызывается каждую секунду для правильно отображения времени
      var t = getTimeRemaining(endtime); //разница между временем
...
__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill(); //перевод ES6


__webpack_require__(/*! nodelist-foreach-polyfill */ "./node_modules/nodelist-foreach-polyfill/index.js"); //или import 'nodelist-foreach-polyfill';//перевод forEach


__webpack_require__(/*! formdata-polyfill */ "./node_modules/formdata-polyfill/formdata.min.js"); // формы


window.addEventListener('DOMContentLoaded', function () {
  //событие срабатывает, когда полностью прогрузился DOM дерево;
  'use strict';
